<template>
  <div class="app-container">
    ...
    <el-table ref="dragTable" v-loading="listLoading" :data="list" border fit highlight-current-row style="width: 100%">
      ...
      <el-table-column min-width="140px" align="center" label="总产值">
        <template slot-scope="scope">
          <span>{{ scope.row.totalvalue }}</span>
        </template>
      </el-table-column>
      <el-table-column min-width="140px" align="center" label="总投入">
        <template slot-scope="scope">
          <span>{{ scope.row.totalinput }}</span>
        </template>
      </el-table-column>
      <el-table-column min-width="140px" align="center" label="总利润">
        <template slot-scope="scope">
          <span>{{ scope.row.totalprofit }}</span>
        </template>
      </el-table-column>
      <el-table-column min-width="140px" align="center" label="售卖日期">
        <template slot-scope="scope">
          <span>{{ scope.row.selldate | parseTime("{y}/{m}/{d}") }}</span>
        </template>
      </el-table-column>
...
import { FetchAnnualBenefitRecords, DeleteAnnualBenefitRecord } from '@/api/fishpond'

export default {
  name: 'AnnualBenefitRecords',
  ...
  methods: {
    fetchList() {
      ...
      FetchAnnualBenefitRecords(this.listQuery).then(response => {
    ...
    handleDelete(item) {
      ...
      this.$confirm('是否删除该条记录', '提示', {
        ...
        DeleteAnnualBenefitRecord(data).then(response => {
          this.$notify({
            title: '已删除',
            type: 'success',
            message: '操作成功',
