<template>
  <div class="app-container">
    ...
    <el-table ref="dragTable" v-loading="listLoading" :data="list" border fit highlight-current-row style="width: 100%">
      ...
      <el-table-column min-width="140px" align="center" label="数量">
        <template slot-scope="scope">
          <span>{{ scope.row.quantity }}</span>
        </template>
      </el-table-column>
      <el-table-column min-width="140px" align="center" label="总额">
        <template slot-scope="scope">
          <span>{{ scope.row.moneyamount }}</span>
        </template>
      </el-table-column>
      <el-table-column min-width="140px" align="center" label="批次">
        <template slot-scope="scope">
          <span>{{ scope.row.batchno }}</span>
        </template>
      </el-table-column>
      <el-table-column min-width="140px" align="center" label="买家">
        <template slot-scope="scope">
          <span>{{ scope.row.buyer }}</span>
        </template>
      </el-table-column>
      <el-table-column min-width="140px" align="center" label="交易日期">
        <template slot-scope="scope">
...
import { FetchSellRecords, DeleteSellRecord } from '@/api/fishpond'
...
export default {
  ...
  methods: {
    fetchList() {
      ...
      FetchSellRecords(this.listQuery).then(response => {
    ...
    handleDelete(item) {
      ...
      this.$confirm('是否删除该条记录', '提示', {
        ...
        DeleteSellRecord(data).then(response => {
          this.$notify({
            title: '已删除',
            type: 'success',
            message: '操作成功',
