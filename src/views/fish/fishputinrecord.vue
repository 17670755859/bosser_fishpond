<template>
  <div class="app-container">
    ...
    <el-table ref="dragTable" v-loading="listLoading" :data="list" border fit highlight-current-row style="width: 100%">
      ...
      <el-table-column min-width="150px" align="center" label="放养品种">
        <template slot-scope="scope">
      ...
      <el-table-column min-width="140px" align="center" label="规格">
        <template slot-scope="scope">
          <span>{{ scope.row.specs }}</span>
        </template>
      </el-table-column>
      <el-table-column align="center" label="投放日期" width="180px">
        <template slot-scope="scope">
          <span>{{ scope.row.putintime | parseTime("{y}/{m}/{d}") }}</span>
        </template>
      </el-table-column>
...
import { FetchFishPutinRecords, DeleteFishPutinRecordById } from '@/api/fishpond'
...
export default {
  ...
  methods: {
    fetchList() {
      ...
      FetchFishPutinRecords(this.listQuery).then(response => {
    ...
    handleDelete(item) {
      ...
      this.$confirm('是否删除该条记录', '提示', {
        ...
        DeleteFishPutinRecordById(data).then(response => {
          this.$notify({
            title: '已删除',
            type: 'success',
            message: '操作成功',
