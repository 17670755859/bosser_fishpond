...
import { FetchDevicesInDeviceGroup, DeleteDeviceFromGroupById, EditDeviceInfo } from '@/api/fishpond'
...
export default {
  ...
  methods: {
    fetchList() {
      ...
      FetchDevicesInDeviceGroup(this.listQuery).then(response => {
    ...
    handleDelete(item) {
      console.log('删除 !!!!')
      this.$confirm('是否将设备从该分组中移除？注：执行此操作不会删除【设备列表】下的该记录，请知晓', '提示', {
        ...
        DeleteDeviceFromGroupById(data).then(response => {
          this.$notify({
            title: '已删除',
            type: 'success',
            message: '操作成功',
