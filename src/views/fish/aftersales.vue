    ...
    <el-dialog :visible.sync="dialogVisibleAdd" :title="'新增'">
      <el-form :model="additem" label-width="150px" label-position="left">
        ...
        <el-form-item label="视频预览图">
          <input id="uploadFileBtnId" ref="uploadFileBtnRef1" style="margin-left: 0px;" type="file" @change="handleUpload">
        </el-form-item>
        <el-form-item style="text-align: center; background: #ffffff">
            <span v-show="additem.videopreviewurl !== '' && additem.videopreviewurl !== null && additem.videopreviewurl !== undefined">
              <img :src="additem.videopreviewurl" style="margin-right: 100px; min-width: 300px; min-height: 300px; max-width: 400px; max-height: 400px;">
            </span>
        </el-form-item>

        <el-form-item label="上传视频">
          <input id="uploadFileBtnId" ref="uploadFileBtnRef2" style="margin-left: 0px;" type="file" @change="handleUploadVideo">
        </el-form-item>
      </el-form>
...
import { GethelpVideo, AddhelpVideo, RemovehelpVideo, UpdatehelpVideo, UploadMedia } from '@/api/fishpond'
...
export default {
  ...
  methods: {
    async handleUpload() {
      var that = this
      const inputFile = await this.$refs.uploadFileBtnRef1.files[0]
      if (inputFile === null || inputFile === undefined) { // 点击的取消
        this.additem.videopreviewurl = null
        return
      }
      ...
      if (this.inputFileInAdd) {
        ...
        reader.onloadend = async function() {
          let res = await that.UploadFiles(reader.result, 0)
          that.additem.videopreviewurl = res
        }
      }
    },
    async handleUploadVideo() {
      ...
      const fileSizeLt = this.inputFileInAdd.size/1024/1024 > 10 ;
      if(fileSizeLt){
        this.$message({
          type: 'error',
          message: '上传视频请小于10MB大小'
        })
        return;
      }
      if (this.inputFileInAdd) {
        const tempInputFile = this.inputFileInAdd
        if (tempInputFile.type !== 'video/quicktime' && tempInputFile.type !== 'video/mp4') {
          alert('不是有效的视频文件！')
          return
        }
        this.addimginfo = Object.assign({}, this.addimginfo, {
        ...
        reader.onloadend = async function() {
          ...
          let res = await that.UploadFiles(reader.result, 1)
    ...
    fetchList() {
      ...
      GethelpVideo(this.listQuery).then(response => {
    ...
    handleDelete(item) {
      this.$confirm('是否删除该条记录', '提示', {
        ...
        RemovehelpVideo(data).then(response => {
          this.$notify({
            title: '已删除',
            type: 'success',
            message: '操作成功',
    ...
    confirmAdd() {
      ...
      if(this.additem.id) {
        UpdatehelpVideo(data).then(response => {
          this.$notify({
            title: '修改成功',
            type: 'success',
            message: '操作成功',
        ...
        AddhelpVideo(data).then(response => {
          this.$notify({
            title: '提交成功',
            type: 'success',
            message: '操作成功',
