<template>
  <div class="app-container">
    ...
    <el-table ref="dragTable" v-loading="listLoading" :data="list" border fit highlight-current-row style="width: 100%">
      ...
      <el-table-column min-width="140px" align="center" label="换水量">
        <template slot-scope="scope">
          <span>{{ scope.row.amount }}</span>
        </template>
      </el-table-column>
      <el-table-column min-width="140px" align="center" label="水温">
        <template slot-scope="scope">
          <span>{{ scope.row.watertemp }}</span>
        </template>
      </el-table-column>
      <el-table-column min-width="140px" align="center" label="换水">
        <template slot-scope="scope">
          <span>{{ scope.row.changewater }}</span>
        </template>
      </el-table-column>
      <el-table-column align="center" label="操作日期" width="180px">
        <template slot-scope="scope">
          <span>{{ scope.row.changetime | parseTime("{y}/{m}/{d}") }}</span>
        </template>
      </el-table-column>
...
import { FetchChangeWaterRecords, DeleteChangeWaterRecordById } from '@/api/fishpond'
...
export default {
  ...
  methods: {
    fetchList() {
      ...
      FetchChangeWaterRecords(this.listQuery).then(response => {
    ...
    handleDelete(item) {
      ...
      this.$confirm('是否删除该条记录', '提示', {
        ...
        DeleteChangeWaterRecordById(data).then(response => {
          this.$notify({
            title: '已删除',
            type: 'success',
            message: '操作成功',
