<template>
  <div class="app-container">
    ...
    <el-table ref="dragTable" v-loading="listLoading" :data="list" border fit highlight-current-row style="width: 100%">
      ...
      <el-table-column min-width="150px" align="center" label="发病品种">
        <template slot-scope="scope">
          <span>{{ scope.row.category }}</span>
        </template>
      </el-table-column>
      <el-table-column min-width="140px" align="center" label="用量">
        <template slot-scope="scope">
          <span>{{ scope.row.dosage }}</span>
        </template>
      </el-table-column>
      <el-table-column min-width="140px" align="center" label="药物名称">
        <template slot-scope="scope">
      ...
      <el-table-column align="center" label="发病日期" width="180px">
        <template slot-scope="scope">
          <span>{{ scope.row.diseasetime | parseTime("{y}/{m}/{d}") }}</span>
        </template>
      </el-table-column>
      <el-table-column align="center" label="施药人" width="180px">
        <template slot-scope="scope">
          <span>{{ scope.row.operator }}</span>
        </template>
      </el-table-column>
      <el-table-column align="center" label="施药日期" width="180px">
        <template slot-scope="scope">
...
import { FetchFishMedicineRecords, DeleteFishMedicineRecordById } from '@/api/fishpond'
...
export default {
  ...
  methods: {
    fetchList() {
      ...
      FetchFishMedicineRecords(this.listQuery).then(response => {
    ...
    handleDelete(item) {
      ...
      this.$confirm('是否删除该条记录', '提示', {
        ...
        DeleteFishMedicineRecordById(data).then(response => {
          this.$notify({
            title: '已删除',
            type: 'success',
            message: '操作成功',
